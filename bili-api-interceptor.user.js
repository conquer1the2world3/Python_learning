      ...
      if (xhr.readyState === 4) {
        if (
          ...
          if (resJson.code === 0 && resJson.data) {
            const availableQualities = resJson.data.dash?.video || [];
            let highestQuality = 80; // 默认1080p
            
            // 如果没有1080p,选择最高可用质量
            if (!availableQualities.some(q => q.bandwidth >= 2000000)) { // 1080p通常需要2Mbps以上带宽
              highestQuality = Math.max(...availableQualities.map(q => q.bandwidth)) >= 1000000 ? 64 : 32; // 720p或480p
            }
            
            // 请求最高可用质量
            setTimeout(() => {
              window.player && window.player.requestQuality(highestQualzity, null);
            }, 1000);
          }
        }
      }
